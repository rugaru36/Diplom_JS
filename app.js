...
function startSolutionProcess() {
  ...
  outputData(
    ...
    GameObject.gameData,
    GameObject.gameData["endTime"]
  )
}

//корректность данных
function checkIfDataCorrect() {
  let stepIsMoreThanAccuracy = +document.getElementById('accuracy').value < +document.getElementById('h').value
  let pIsNotFasterThanE = +document.getElementById('pursuiterVectorLength').value <= +document.getElementById('escaperVectorLength').value
  let pRadIsNotMoreThanERad =
...
function outputData(EscaperCoordinates, PursuiterCoordinates, controlData, time) {
  ...
  if (document.getElementById('testModCheck').checked) {
    ...
    var durationOfAnimation = time / document.getElementById('speed').value * 4
  ...
  setTimeout(function () {
    ...
    document.getElementById('result').innerHTML += `Количество итераций: ${controlData["endTime"] / 1000}`
