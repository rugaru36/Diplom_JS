...
class Player {
  ...
  moveToWantedPoint(stepSize) {
    ...
    if (this.radius == 0) {
      ...
      if (angleToWantedPoint > Math.PI) {
        angleToWantedPoint -= 2 * Math.PI
      } else if (angleToWantedPoint < -1 * Math.PI) {
        angleToWantedPoint += 2 * Math.PI
      }
...
class Pursuiter extends Player {
  ...
  calculateNextPoint(escaperCoordinates, h) {
    var Lvector = [escaperCoordinates[0] - this.currentCoordinates[0], escaperCoordinates[1] - this.currentCoordinates[1]]
...
class Game {
  ...
  checkIsSolved() {
    ...
    //центры
    var radCenter1 = [this.p_player.currentCoordinates[0] + buffVector1[0], this.p_player.currentCoordinates[1] + buffVector1[1]]
    var radCenter2 = [this.p_player.currentCoordinates[0] + buffVector2[0], this.p_player.currentCoordinates[1] + buffVector2[1]]
    //вектор от убегающего к центру
    var vectorToFirstRadPoint = [radCenter1[0] - this.e_player.currentCoordinates[0], radCenter1[1] - this.e_player.currentCoordinates[1]]
    var vectorToSecondRadPoint = [radCenter2[0] - this.e_player.currentCoordinates[0], radCenter2[1] - this.e_player.currentCoordinates[1]]
